...
func LoadSettings() (app *Set, err error) {
...
func InitializeDatabase(app *Set) (db *gorm.DB, err error) {
	option := app.Database
...
func InitializeRedis(app *Set) (client *redis.Client, err error) {
	option := app.Redis
...
func InitializeCookie(app *Set) *cookie.Cookie {
	return cookie.New(app.Cookie, http.SameSiteStrictMode)
}

// InitializeAuthx 创建认证
func InitializeAuthx(app *Set) *authx.Authx {
